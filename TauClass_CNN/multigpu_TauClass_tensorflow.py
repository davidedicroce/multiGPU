...
if __name__ == '__main__':
    ...
    epochs = args.epochs
    expt_name = 'TauClass-opendata_ResNet_blocks%d_x1_epochs%d'%(resblocks, epochs)
...
def extract_fn(data):
    ...
    feature_description = {
        'X_jets': tf.io.FixedLenFeature([125*granularity*125*granularity*13], tf.float32),
...
def map_fn(data):
    ...
    x = tf.reshape(data['X_jets'], (125*granularity,125*granularity,13))[...,0:13]
    y = tf.one_hot(tf.cast(data['y'], tf.uint8), classes)
    return x, y

def x_fn(data):
    return tf.reshape(data['X_jets'], (125*granularity,125*granularity,13))[...,0:13]
