...
if __name__ == '__main__':
    ...
    probs = pred[:,1]
    #binary_preds=[]
    true_label = np.squeeze(np.array(list(test_data[1].as_numpy_iterator())))
    print(pred[np.isfinite(pred)].shape)
    print(probs[np.isfinite(probs)].shape)
    fpr, tpr, _ = roc_curve(true_label[np.isfinite(true_label)], np.squeeze(np.array(probs[np.isfinite(probs)])))
